...
/**
 * Compile font wrapper using cross-platform build system
 */
...
/**
 * Main compilation function for font wrapper
 */
async function compileFontWrapper() {
    console.log('🔨 Compiling font wrapper...\n');

    try {
        // Create platform-specific builder
        const builder = createPlatformBuilder();

        // Get font wrapper configuration
        const wrappers = builder.getWrapperConfigurations();
        const fontWrapper = wrappers.find(w => w.name === 'font-wrapper');

        if (!fontWrapper) {
            throw new Error('Font wrapper configuration not found');
        }

        console.log(`🔍 Platform: ${builder.platform.os} (${builder.platform.arch})`);
        console.log(`📁 Source: ${fontWrapper.source}`);
        console.log(`📦 Output: ${fontWrapper.output}\n`);
        ...
        // Build font wrapper
        const success = await builder.buildWrapper(fontWrapper);

        if (success) {
            console.log('\n🎉 Font wrapper compiled successfully!');
            console.log(`📦 Output: ${fontWrapper.output}`);
            process.exit(0);
        } else {
            console.error('\n❌ Font wrapper compilation failed');
            process.exit(1);
        }
...
async function compileLegacy() {
    console.log('🔄 Running font wrapper compilation in legacy mode...\n');

    const { $ } = await import("bun");
    const { existsSync, mkdirSync } = await import("fs");

    const ASSETS_DIR = "assets";
    const WRAPPERS_DIR = join(packageRoot, "src/wrappers/fonts");
    ...
    // Compile font-wrapper.c
    const sourceFile = join(WRAPPERS_DIR, "font-wrapper.c");
    const outputFile = join(ASSETS_DIR, "lib", "font-wrapper.dylib");

    console.log("🔨 Compiling font wrappers...");
    ...
    try {
        ...
        console.log("✅ Font wrappers compiled successfully!");
 ...
 */
function printUsage() {
    console.log('Usage: bun compile-fonts.js [options]');
    console.log('');
    console.log('Options:');
