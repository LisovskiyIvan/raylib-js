...
/**
 * Compile render texture wrapper using cross-platform build system
 */
...
/**
 * Main compilation function for render texture wrapper
 */
async function compileRenderTextureWrapper() {
  ...
  try {
    ...
    // Get render texture wrapper configuration
    const wrappers = builder.getWrapperConfigurations();
    const renderTextureWrapper = wrappers.find(w => w.name === 'render-texture-wrapper');

    if (!renderTextureWrapper) {
      throw new Error('Render texture wrapper configuration not found');
    }

    console.log(`🔍 Platform: ${builder.platform.os} (${builder.platform.arch})`);
    console.log(`📁 Source: ${renderTextureWrapper.source}`);
    console.log(`📦 Output: ${renderTextureWrapper.output}\n`);
    ...
    // Build render texture wrapper
    const success = await builder.buildWrapper(renderTextureWrapper);

    if (success) {
      console.log('\n🎉 Render texture wrapper compiled successfully!');
      console.log(`📦 Output: ${renderTextureWrapper.output}`);
      process.exit(0);
    } else {
      console.error('\n❌ Render texture wrapper compilation failed');
      process.exit(1);
    }
...
async function compileLegacy() {
  console.log('🔄 Running render texture wrapper compilation in legacy mode...\n');
  ...
  // Compile render-texture-wrapper.c
  const sourceFile = join(WRAPPERS_DIR, "render-texture-wrapper.c");
  const outputFile = join(ASSETS_DIR, "lib", "render-texture-wrapper.dylib");
 ...
 */
function printUsage() {
  console.log('Usage: bun compile-render-texture.js [options]');
  console.log('');
  console.log('Options:');
