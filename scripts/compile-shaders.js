...
/**
 * Compile shader wrapper using cross-platform build system
 */
...
/**
 * Main compilation function for shader wrapper
 */
async function compileShaderWrapper() {
    console.log('🔨 Compiling shader wrapper...\n');

    try {
        // Create platform-specific builder
        const builder = createPlatformBuilder();

        // Get shader wrapper configuration
        const wrappers = builder.getWrapperConfigurations();
        const shaderWrapper = wrappers.find(w => w.name === 'shader-wrapper');

        if (!shaderWrapper) {
            throw new Error('Shader wrapper configuration not found');
        }

        console.log(`🔍 Platform: ${builder.platform.os} (${builder.platform.arch})`);
        console.log(`📁 Source: ${shaderWrapper.source}`);
        console.log(`📦 Output: ${shaderWrapper.output}\n`);
        ...
        const success = await builder.buildWrapper(shaderWrapper);

        if (success) {
            console.log('\n🎉 Shader wrapper compiled successfully!');
            console.log(`📦 Output: ${shaderWrapper.output}`);
            process.exit(0);
        } else {
            console.error('\n❌ Shader wrapper compilation failed');
            process.exit(1);
        }
...
async function compileLegacy() {
    console.log('🔄 Running shader wrapper compilation in legacy mode...\n');

    const { $ } = await import("bun");
    const { existsSync, mkdirSync } = await import("fs");

    const ASSETS_DIR = "assets";
    const WRAPPERS_DIR = join(packageRoot, "src/wrappers/shaders");
    ...
    try {
        ...
        console.log("✅ Shader wrappers compiled successfully!");
 ...
 */
function printUsage() {
    console.log('Usage: bun compile-shaders.js [options]');
    console.log('');
    console.log('Options:');
