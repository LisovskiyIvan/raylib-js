...
export default class Raylib {
    ...
    public drawTriangle(v1: Vector2, v2: Vector2, v3: Vector2, color: number): RaylibResult<void> {
        return this.requireInitialized()
            ...
            .andThen(() => {
                // Try the native DrawTriangle function first
                const nativeResult = this.safeFFICall('draw triangle', () =>
                    this.rl.DrawTriangle(Math.floor(v1.x), Math.floor(v1.y), Math.floor(v2.x), Math.floor(v2.y), Math.floor(v3.x), Math.floor(v3.y), color)
                )

                // If native function fails or doesn't render properly on Windows, use fallback
                if (nativeResult.isErr()) {
                    return this.drawTriangleFilled(v1, v2, v3, color)
                }
    ...
    public drawTriangleLines(v1: Vector2, v2: Vector2, v3: Vector2, color: number): RaylibResult<void> {
        return this.requireInitialized()
            .andThen(() => validateAll(
                validateFinite(v1.x, 'v1.x'),
                validateFinite(v1.y, 'v1.y'),
            ...
            .andThen(() => this.safeFFICall('draw triangle lines', () =>
                this.rl.DrawTriangleLines(Math.floor(v1.x), Math.floor(v1.y), Math.floor(v2.x), Math.floor(v2.y), Math.floor(v3.x), Math.floor(v3.y), color)
            ))
    }

    // TODO: fix this function on windows
    public drawTriangleFilled(v1: Vector2, v2: Vector2, v3: Vector2, color: number): RaylibResult<void> {
        return this.requireInitialized()
            .andThen(() => validateAll(
                validateFinite(v1.x, 'v1.x'),
                validateFinite(v1.y, 'v1.y'),
            ...
            .andThen(() => this.safeFFICall('draw triangle filled', () => {
                // Sort vertices by Y coordinate
                const vertices = [v1, v2, v3].sort((a, b) => a.y - b.y)
                ...
                for (let y = Math.floor(top.y); y <= Math.floor(bottom.y); y++) {
                    ...
                    if (y <= mid.y) {
                        ...
                        if (bottom.y !== top.y) {
                            x1 = top.x + (bottom.x - top.x) * (y - top.y) / (bottom.y - top.y)
                        } else {
                            x1 = top.x
                        }

                        if (mid.y !== top.y) {
                            x2 = top.x + (mid.x - top.x) * (y - top.y) / (mid.y - top.y)
                        } else {
                            x2 = top.x
                        }
                    } else {
                        ...
                        if (bottom.y !== mid.y) {
                            x2 = mid.x + (bottom.x - mid.x) * (y - mid.y) / (bottom.y - mid.y)
                        } else {
                            x2 = mid.x
                        }
                    }
                    ...
                    if (Math.abs(x2 - x1) > 0.5) {
                        this.rl.DrawLine(Math.floor(x1), y, Math.floor(x2), y, color)
                    }
                }
            }))
    }
    ...
    public drawRectanglePro(rec: Rectangle, origin: Vector2, rotation: number, color: number): RaylibResult<void> {
        return this.requireInitialized()
            ...
            .andThen(() => this.safeFFICall('draw rectangle pro', () => {
                ...
                this.rl.DrawRectanglePro(ptr(recArray), ptr(originArray), rotation, color)
            }))
    }
        ...
        return this.requireInitialized()
            ...
            .andThen(() => this.safeFFICall('check collision circles', () => {
                try {
                    ...
                    const result = this.rl.CheckCollisionCircles(ptr(center1Buffer), radius1, ptr(center2Buffer), radius2)

                    // If native function returns false, use fallback implementation
                    if (!result) {
                        return this.checkCollisionCirclesFallback(center1, radius1, center2, radius2)
                    }
                    return result
                } catch (error) {
                    return this.checkCollisionCirclesFallback(center1, radius1, center2, radius2)
    ...
    private checkCollisionCirclesFallback(center1: Vector2, radius1: number, center2: Vector2, radius2: number): boolean {
        ...
        return this.requireInitialized()
            ...
            .andThen(() => this.safeFFICall('check collision circle rectangle', () => {
                ...
                try {
                    ...
                    const result = this.rl.CheckCollisionCircleRec(ptr(centerBuffer), radius, ptr(recBuffer))
    ...
    private checkCollisionCircleRecFallback(center: Vector2, radius: number, rec: Rectangle): boolean {
    ...
    public checkCollisionCircleLine(center: Vector2, radius: number, p1: Vector2, p2: Vector2): RaylibResult<boolean> {
        return this.requireInitialized()
            ...
            .andThen(() => this.safeFFICall('check collision circle line', () => {
                ...
                return this.rl.CheckCollisionCircleLine(ptr(centerBuffer), radius, ptr(p1Buffer), ptr(p2Buffer))
            }))
    }
        ...
        return this.requireInitialized()
            ...
            .andThen(() => this.safeFFICall('check collision point rectangle', () => {
                try {
                    ...
                    const result = this.rl.CheckCollisionPointRec(ptr(pointBuffer), ptr(recBuffer))
    ...
    private checkCollisionPointRecFallback(point: Vector2, rec: Rectangle): boolean {
        ...
        return this.requireInitialized()
            ...
            .andThen(() => this.safeFFICall('check collision point circle', () => {
                try {
                    ...
                    const result = this.rl.CheckCollisionPointCircle(ptr(pointBuffer), ptr(centerBuffer), radius)

                    if (!result) {
                        return this.checkCollisionPointCircleFallback(point, center, radius)
                    }
                    return result
                } catch (error) {
                    return this.checkCollisionPointCircleFallback(point, center, radius)
    ...
    private checkCollisionPointCircleFallback(point: Vector2, center: Vector2, radius: number): boolean {
    ...
    public checkCollisionPointTriangle(point: Vector2, p1: Vector2, p2: Vector2, p3: Vector2): RaylibResult<boolean> {
        return this.requireInitialized()
            ...
            .andThen(() => this.safeFFICall('check collision point triangle', () => {
                ...
                return this.rl.CheckCollisionPointTriangle(ptr(pointBuffer), ptr(p1Buffer), ptr(p2Buffer), ptr(p3Buffer))
            }))
    }
    ...
    public drawTriangle3D(v1: Vector3, v2: Vector3, v3: Vector3, color: number): RaylibResult<void> {
        return this.requireInitialized()
            ...
            .andThen(() => this.safeFFICall('draw triangle 3D', () => {
                ...
                this.rl.DrawTriangle3D(ptr(v1Array), ptr(v2Array), ptr(v3Array), color)
            }))
    }
    ...
    public drawCylinder(position: Vector3, radiusTop: number, radiusBottom: number, height: number, slices: number, color: number): RaylibResult<void> {
        return this.requireInitialized()
            ...
            .andThen(() => this.safeFFICall('draw cylinder', () => {
                ...
                this.rl.DrawCylinder(ptr(posArray), radiusTop, radiusBottom, height, slices, color)
            }))
    }
    ...
    public drawCapsule(startPos: Vector3, endPos: Vector3, radius: number, slices: number, rings: number, color: number): RaylibResult<void> {
        return this.requireInitialized()
            ...
            .andThen(() => this.safeFFICall('draw capsule', () => {
                ...
                this.rl.DrawCapsule(ptr(startArray), ptr(endArray), radius, slices, rings, color)
            }))
    }
    ...
    public getModelInformation(model: Model): RaylibResult<{ meshCount: number, materialCount: number, boundingBox: BoundingBox }> {
        return this.requireInitialized()
            .andThen(() => {
                if (model.slotIndex < 0) {
                    return new Err(validationError('Invalid model slot index'))
        ...
        return this.requireInitialized()
            ...
            .andThen(() => {
                ...
                if (isValidResult.isErr()) {
                    console.warn('Cannot validate model slot, using fallback cube representation')
                    return this.drawCube(position, scale * 2, scale * 2, scale * 2, tint)
                }
                if (!isValidResult.unwrap()) {
                    console.warn('Invalid model slot, using fallback cube representation')
                    return this.drawCube(position, scale * 2, scale * 2, scale * 2, tint)
                }
    ...
    public drawModelWires(model: Model, position: Vector3, scale: number, tint: number): RaylibResult<void> {
        return this.requireInitialized()
            ...
            .andThen(() => {
                ...
                const isValidResult = this.isModelSlotValid(model.slotIndex)
                if (isValidResult.isErr()) {
                    console.warn('Cannot validate model slot, using fallback wireframe cube')
    ...
    private drawWireframeCube(position: Vector3, size: number, tint: number): RaylibResult<void> {
    ...
    public isModelSlotValid(slotIndex: number): RaylibResult<boolean> {
        return this.requireInitialized()
            .andThen(() => this.safeFFICall('is model slot valid', () => {
    ...
    public checkCollisionSpheres(center1: Vector3, radius1: number, center2: Vector3, radius2: number): RaylibResult<boolean> {
        return this.requireInitialized()
            ...
            .andThen(() => this.safeFFICall('check collision spheres', () => {
                ...
                return this.rl.CheckCollisionSpheres(ptr(center1Buffer), radius1, ptr(center2Buffer), radius2)
            }))
    }
    ...
    public checkCollisionBoxSphere(box: BoundingBox, center: Vector3, radius: number): RaylibResult<boolean> {
        return this.requireInitialized()
            ...
            .andThen(() => this.safeFFICall('check collision box sphere', () => {
                ...
                return this.rl.CheckCollisionBoxSphere(ptr(boxArray), ptr(centerBuffer), radius)
            }))
    }
